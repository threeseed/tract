...
impl<T, const N: usize> SmallVec<T, N> {
    ...
    /// Constructs a new, empty `SmallVec<T, N>` with at least the specified capacity.
    ///
    /// The vector will be able to hold at least `capacity` elements without
    /// reallocating. This method is allowed to allocate for more elements than
    /// `capacity`. If `capacity` is 0 or less than or equal to N, the vector will not allocate.
    #[inline]
    pub fn with_capacity(capacity: usize) -> Self {
        let mut this = Self::new();
        if capacity > N {
            let len = this.len();
            this.handle().reserve(capacity - len);
        }
        this
    }
    ...
    /// Returns the inline capacity (the value of N).
    #[inline]
    ...
    pub fn insert(&mut self, index: usize, element: T) {
        self.handle().insert(index, element);
    }
    ...
    {
        self.handle().dedup_by(same_bucket);
    }
    ...
    where
        F: FnMut() -> T,
    {
        self.handle().resize_with(new_len, f);
    }
    ...
    where
        T: Clone,
    {
        self.handle().resize(new_len, value);
    }
    ...
    where
        T: Clone,
    {
        self.handle().extend(other.iter().cloned());
    }
    ...
    /// Creates a `SmallVec` from a `Vec`.
    pub fn from_vec(vec: Vec<T>) -> Self {
        let mut this = Self::new();
        let mut handle = this.handle();
        handle.extend(vec);
        drop(handle);
        this
    }
...
impl<T, const N: usize> core::ops::Index<core::ops::Range<usize>> for SmallVec<T, N> {
    type Output = [T];
...
impl<T, const N: usize> core::ops::IndexMut<core::ops::Range<usize>> for SmallVec<T, N> {
    #[inline]
...
impl<T, const N: usize> core::ops::Index<core::ops::RangeFrom<usize>> for SmallVec<T, N> {
    type Output = [T];
...
impl<T, const N: usize> core::ops::IndexMut<core::ops::RangeFrom<usize>> for SmallVec<T, N> {
    #[inline]
...
impl<T, const N: usize> core::ops::Index<core::ops::RangeTo<usize>> for SmallVec<T, N> {
    type Output = [T];
...
impl<T, const N: usize> core::ops::IndexMut<core::ops::RangeTo<usize>> for SmallVec<T, N> {
    #[inline]
        ...
        if let Some(item) = iter.next() {
            ...
            fn from_iter_impl<T, const N: usize>(
                ...
                unsafe {
                    ...
                    ptr::write(vec.as_mut_ptr().add(N), item);
                    vec.set_len(N + 1);
                }
                vec.extend(iter);
                init_from_vec(this, vec);
...
impl<T: Clone, const N: usize> SmallVecHandle<'_, T, N> {
    ...
    pub fn resize(&mut self, new_len: usize, value: T) {
        ...
        if new_len > len {
            self.reserve(new_len - len);
            unsafe {
                let mut cur = self.as_mut_ptr().add(len);
                let end = self.as_mut_ptr().add(new_len);
                while cur != end.sub(1) {
...
/// Trait for converting slices to SmallVec.
pub trait ToSmallVec<T, const N: usize> {
    /// Converts the slice to a SmallVec.
    fn to_smallvec(&self) -> SmallVec<T, N>;
}

impl<T: Clone, const N: usize> ToSmallVec<T, N> for [T] {
    fn to_smallvec(&self) -> SmallVec<T, N> {
