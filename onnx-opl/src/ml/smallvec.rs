...
impl<T, const N: usize> SmallVec<T, N> {
    ...
    pub const fn new() -> Self {
        ...
        Self { ptr: NonNull::dangling(), cap: N, len: 0, local }
    }
    ...
    pub const fn as_ptr(&self) -> *const T {
        if self.is_local() { self.local.as_ptr().cast() } else { self.ptr.as_ptr() }
    }
    ...
    pub const fn as_mut_ptr(&mut self) -> *mut T {
        if self.is_local() { self.local.as_mut_ptr().cast() } else { self.ptr.as_ptr() }
    }
...
impl<T, const N: usize> IntoIterator for SmallVec<T, N> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        IntoIter { vec: ManuallyDrop::new(self), cur: 0 }
    }
}
...
impl<T, const N: usize> SmallVecHandle<'_, T, N> {
        ...
        F: FnMut(&mut T) -> bool,
    {
        ...
        let mut g =
            BackshiftOnDrop { v: &mut *self.0, processed_len: 0, deleted_cnt: 0, original_len };
